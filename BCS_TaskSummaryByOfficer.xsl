...
<root>
	<!--   2020/10/14 Update J.Ma #4227
                - Added SQLParameter and XSLT Parameter for ClearanceStatusDisp Field
                - Added SQL query for UCRClearanceStatusG with conditional for ClearanceStatus. (Not required field)
                - Added XSLT output to the Task - Clearance Status
                - Added Field UI for Clearance Status for Print Report
                - Added DataWindowDefinition  for ClearanceStatus


     -->
	<VersionInfo>
		<Filename>TaskSummaryByOfficer.xml</Filename>
		<Revision>b09d8423f7b4f7b83148ece7cfe27728ad87b0f3</Revision>
		<NMLVersion>6.01.03.01.36</NMLVersion>
		<Developer>Niche</Developer>
		<NicheBuildName>BexarCounty</NicheBuildName>
		<BuildTime>2020/09/29 09:44:59</BuildTime>
	</VersionInfo>
	...
	<SQLSelectXSLT><![CDATA[<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
				...
				<xsl:template match="/">
					...
					--  File revision: b09d8423f7b4f7b83148ece7cfe27728ad87b0f3
					--  NML version: 6.01.03.01.36
					--  Developer: Niche
					--  Niche build name: BexarCounty
					--  Build time: 2020/09/29 09:44:59
					----------------------------------------------------------
	...
	<ReportXSL>
		<![CDATA[<?xml version="1.0" encoding="Windows-1252"?>
			<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:nl="urn:nicheLanguage" xmlns:nicheFunctions="urn:nicheFunctions" xmlns:pp="urn:nichePreprocessor" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
				...
				<xsl:key name="Officer" match="/DATASETLIST/DATASET/ROW/GTask__TaskAssignedToRId" use="." />

				<xsl:template match="/">
					<HTML>
						<xsl:variable name="ReportTitle">Officer task summary report</xsl:variable>
						...
						<xsl:element name="BODY">
							...
							<TABLE>
								...
								<xsl:for-each select="/DATASETLIST/DATASET/ROW[GTask__TaskNumber][generate-id(GTask__TaskAssignedToRId) = generate-id(key('Officer', GTask__TaskAssignedToRId)[1])]">
									...
									<TR>
										...
										<TD class="total count"><xsl:value-of select="count(/DATASETLIST/DATASET/ROW[GTask__TaskAssignedToRId = $OfficerId])" /></TD>
										<xsl:for-each select="/DATASETLIST/DATASET/ROW[DBValue]">
											<TD class="count"><xsl:value-of select="count(/DATASETLIST/DATASET/ROW[GTask__TaskAssignedToRId = $OfficerId][GTask__StatusG = current()/GUIValue])" /></TD>
										</xsl:for-each>
									</TR>
								</xsl:for-each>
								<TR>
									<TD class="totalrow">Total:</TD>
									<TD class="totalrow total count"><xsl:value-of select="count(/DATASETLIST/DATASET/ROW[GTask__TaskNumber])" /></TD>
									<xsl:for-each select="/DATASETLIST/DATASET/ROW[DBValue]">
										<TD class="totalrow count"><xsl:value-of select="count(/DATASETLIST/DATASET/ROW[GTask__StatusG = current()/GUIValue])" /></TD>
									</xsl:for-each>
								</TR>
							</TABLE>
							...
							<xsl:for-each select="/DATASETLIST/DATASET/ROW[GTask__TaskNumber][generate-id(GTask__TaskAssignedToRId) = generate-id(key('Officer', GTask__TaskAssignedToRId)[1])]">
								...
								<DIV class="officerBox">
									...
									<H2>Task Details: <A href="nds://{GTask__TaskAssignedToRId}"><xsl:value-of select="GTask__TaskAssignedToRId_L" /></A></H2>

									<xsl:for-each select="/DATASETLIST/DATASET/ROW[GTask__TaskAssignedToRId = current()/GTask__TaskAssignedToRId]">
...
<!--Updated Row 20201014  J.Ma#4227							-->
											...
											<tr>
												...
												<xsl:choose>
													<xsl:when test="GTask__SupervisorApprovalRequired = 'True'">
												...
												<xsl:choose>
													<xsl:when test="GTask__InitiatorApprovalRequired = 'True'">
														<td><nl:mls><e>Yes</e><f>Oui</f></nl:mls></td>
													</xsl:when>
													<xsl:otherwise>
														<td><nl:mls><e>No</e><f>Non</f></nl:mls></td>
											...
											<tr>
												...
												<td colspan="3"><xsl:value-of select="GTask__TaskInitiatedByRId_L" /></td>
											...
											<tr>
												...
												<td colspan="3"><xsl:value-of select="GTask__TaskAssignedToRId_L" /></td>
												<td><I><nl:mls><e>Time:</e><f>Heure:</f></nl:mls></I></td>
												<td><xsl:value-of select="GTask__TaskAssignedToAssignedTimeTZV2G" /></td>
											</tr>
											<tr>
												<td><I><nl:mls><e>Approved by:</e><f>Approuvée par:</f></nl:mls></I></td>
												<td colspan="3"><xsl:value-of select="GTask__TaskApprovalRId_L" /></td>
												<td><I><nl:mls><e>Time:</e><f>Heure:</f></nl:mls></I></td>
												<td><xsl:value-of select="GTask__TaskApprovalApprovalTimeTZV2G" /></td>
											</tr>
											<tr>
											...
											<tr>
												<xsl:variable name="Occurrences" select="DATASET/ROW[GOccurrence__LabelShort]" />
												<td><I>Associated occurrence<xsl:if test="count($Occurrences) &gt; 1">s</xsl:if>:</I></td>
												<td colspan="5">
				...
				<xsl:template name="StyleClasses">
					<xsl:element name="style">
						...
						<xsl:text>
							...
							caption			{ font-weight:bold; padding:2px; border:solid black; border-width:2px; background-color:#DDD; font-size:1.4em; }
							...
							h2				{ font-family:Arial,sans-serif; font-weight:bold; font-style:normal; padding:2px; border:solid black; border-width:0 0 2px 0; background-color:#DDD; font-size:1.4em; margin:0; }
							...
							.officerBox		{ border:solid 2px black; width:99%; margin:0; text-align:center; }
		...
		<DSL>
			<Metadata>
				<Entity Name="Parameters">
					...
					<SetField Name="TaskType" ChoiceListName="cl_TaskType" ChoiceListSource="NML" />
					<Field Name="TaskTypeDisp" />
					<SetField Name="ClearanceStatus" ChoiceListName="cl_UCRIncidentClearanceStatus" ChoiceListSource="NML" />
			...
			<Entity Name="Parameters" DSLBased="True">
				...
				<OnFieldChange Name="ClearanceStatusG" Action="CopyValueOnUpdate">
				...
				<Field Name="TaskTypeDisp" />
				<Field Name="ClearanceStatusG" ExportName="ClearanceStatus" ExportType="StorableValue" />
				<Field Name="ClearanceStatusDisp" />
			</Entity>
		</DSL>
		...
		<USL>
			<![CDATA[
				view XSLTViewTemplate
				{
					...
					flowGrid (style: flg_NoAutoSingle, maxColumns: 2, content: {
						column (span: 1, content: qaGrid (content: {
							qaRow (style: qar_QuestionnaireStretchedFullAnswer, fieldName: 'StartTimeG', headerText: 'Initiated from:')
							qaRow (style: qar_QuestionnaireStretchedFullAnswer, fieldName: 'EndTimeG', headerText: 'Initiated to:')
							qaRow (style: qar_QuestionnaireStretchedFullAnswer, fieldName: 'TaskTypeG', headerText: 'Task type:')
							qaRow (style: qar_QuestionnaireStretchedFullAnswer, fieldName: 'ClearanceStatusG', headerText: 'Clearance Status:')
						}))
					})
				}
			]]>
